...
const MAX_CLINIC_PHOTO_SIZE = 4 * 1024 * 1024;
...
export function validateProviderClinicPhotoFile(params: {
	buffer: Buffer;
	mimeType: string;
}) {
	...
	if (buffer.length === 0) {
		throw new BadRequestError("Clinic photo file is empty");
	}

	if (buffer.length > MAX_CLINIC_PHOTO_SIZE) {
		throw new BadRequestError("Clinic photo exceeds the 4MB limit");
	}

	if (!isAllowedPhotoMimeType(mimeType)) {
		throw new BadRequestError("Unsupported clinic photo file type");
	}

	const fileType = allowedPhotoTypes[mimeType];

	if (!fileType.matches(buffer)) {
		throw new BadRequestError("Clinic photo content does not match its type");
	}

	if (hasSuspiciousTextPayload(buffer)) {
		throw new BadRequestError("Clinic photo content is not allowed");
	}
